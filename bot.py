...
def trading_dashboard():
    ...
    cur = conn.cursor()
    cur.execute("SELECT symbol, signal_type, entry_price, current_price FROM trading_signals ORDER BY signal_time DESC LIMIT 20;")
    rows = cur.fetchall()
    conn.close()
    return jsonify([
        {"symbol": r[0], "signal_type": r[1], "entry_price": float(r[2]), "current_price": float(r[3])}
        for r in rows
    ])
